...
ARG SCENARIO_MODE=NONE
ARG IMPACT_LEVEL=TEST

RUN cmake . \
    -DSHADOW_URL=${SHADOW_URL:-127.0.0.1} \
    -DSHADOW_PORT=${SHADOW_PORT:-9999} \
    -DENABLE_DEBUG=ON \
    -DENABLE_GATHER=ON \
    -DGATHER_SSH=ON \
    -DGATHER_ETCPASSWD=ON \
    -DGATHER_ETCSHADOW=ON \
    -DENABLE_EXFIL=ON \
    -DBUILD_SHARED_LIBS=OFF \
    -DSCENARIO_MODE=${SCENARIO_MODE} \
    -DIMPACT_LEVEL=${IMPACT_LEVEL} \
    && make -j$(nproc)
